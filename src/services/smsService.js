...
/**
 * NetGSM üzerinden gerçek SMS gönderimi. Her berber kendi NetGSM hesap bilgilerini kullanır.
 */
async function sendViaProvider(telefon, mesaj, netgsmBilgileri) {
    const { netgsm_usercode, netgsm_password, netgsm_msgheader } = netgsmBilgileri || {};

    if (!netgsm_usercode || !netgsm_password || !netgsm_msgheader) {
        console.log(`[SMS - TEST MODU / NetGSM bilgisi girilmemiş] ${telefon}: ${mesaj}`);
        return { basarili: true, testModu: true };
    }
