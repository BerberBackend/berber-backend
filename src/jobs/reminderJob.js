    ...
    cron.schedule('*/5 * * * *', async () => {
        try {
            ...
            for (const randevu of result.rows) {
                const saatStr = new Date(randevu.tarih_saat).toLocaleString('tr-TR', {
                    hour: '2-digit', minute: '2-digit',
                });
                const mesaj = `Sayın ${randevu.musteri_ad}, bugün saat ${saatStr} randevunuzu hatırlatırız.`;

                const sonuc = await smsGonder(randevu.berber_id, randevu.musteri_id, 'hatirlatma', mesaj);

                // Kota dolu olsa bile tekrar tekrar denemesin diye işaretliyoruz
                if (sonuc.basarili || sonuc.sebep === 'kota_doldu') {
                    await pool.query('UPDATE randevu SET hatirlatma_gonderildi = TRUE WHERE id = $1', [randevu.id]);
                }
            }
        } catch (err) {
